/*
PillBot MTE 100 Design Project Code
Code written by Ilan Rajpar, Uday Roy, Aly Ahmed, Bishoy Abdelnour
*/
...
task main()
{
  ...
  // store data from text file into arrays
  ReadFromFile(fin, pill_1, pill_2, pill_3, wait_time_hrs);

  // loops to dispense pills
  for(int times = 0; times < 3; times++)
  {
    float time_in_Hrs = wait_time_hrs[times];
    // make sure to use a shorter wait time for the demo
    time1[T1] = 0;
    ...
    // plays alert until user clicks button
  }

  // boolean variables for refilling the containers
  ...
  // move the colour sensor up and down to check the level of pills
  ...
  // display messages and sound alerts for containers that need to be refilled
